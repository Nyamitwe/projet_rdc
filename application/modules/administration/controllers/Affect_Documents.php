 ...
 */
   ...
   class Affect_Documents  extends CI_Controller 
   {
    ...
    $this->load->view('Affect_Documents_View',$data);
    }
   ...
   $stages=$this->Model->getRequete('SELECT * from pms_stage join pms_process_stage on pms_stage.STAGE_ID=pms_process_stage.STAGE_ID join pms_process on pms_process_stage.PROCESS_ID=pms_process.PROCESS_ID join pms_process_service on pms_process_service.STAGE_ID=pms_process_stage.STAGE_ID  where 1 and pms_process_service.ID_POSTE=1  order by DESCRIPTION_STAGE');
    ...
    public function save()
    {
     ...
     $doc=$this->Model->getRequete('SELECT * from pms_documents where ID_TYPE_DOCUMENT=1 order by DESC_DOCUMENT');
      ...
      $pro=$this->Model->getOne('pms_documents_stage',array('ID_DOCUMENT'=>$this->input->post('doc'.$i.''),'ID_POSTE'=>$POSTE_ID,'PROCESS_ID'=>$PROCESS_ID,'STAGE_ID'=>$STAGE_ID));
      ...
      $data_arr=array('ID_DOCUMENT'=>$this->input->post('doc'.$i.''),
       ...
       $this->Model->create('pms_documents_stage',$data_arr);
      ...
      redirect(base_url('administration/Affect_Documents/liste'));
    }
   ...
   $this->load->view('Affect_Documents_List_View',$data);
  ...
  function get_liste()
  {
   $SERVICE_ID=$this->input->post('SERVICE_ID');
    ...
    $query_principal="SELECT pms_documents_stage.ID,pms_documents.DESC_DOCUMENT,pms_process.DESCRIPTION_PROCESS,pms_service.DESCRIPTION,pms_stage.DESCRIPTION_STAGE, pms_poste_service.POSTE_DESCR FROM pms_documents_stage join pms_process on pms_process.PROCESS_ID=pms_documents_stage.PROCESS_ID JOIN pms_stage on pms_stage.STAGE_ID=pms_documents_stage.STAGE_ID join pms_documents on pms_documents.ID_DOCUMENT=pms_documents_stage.ID_DOCUMENT join pms_poste_service on pms_documents_stage.ID_POSTE=pms_poste_service.ID_POSTE join pms_service on pms_documents_stage.SERVICE_ID=pms_service.SERVICE_ID WHERE 1 ".$crit."";
   ...
   $search = !empty($_POST['search']['value']) ? (" AND (pms_poste_service.POSTE_DESCR LIKE '%$var_search%' or pms_process.DESCRIPTION_PROCESS LIKE '%$var_search%' or pms_service.DESCRIPTION LIKE '%$var_search%' or pms_stage.DESCRIPTION_STAGE LIKE '%$var_search%' or pms_documents.DESC_DOCUMENT LIKE '%$var_search%' )") : '';
    ...
    foreach ($fetch_data as $row) 
     ...
     // $proces=$this->Model->getOne('pms_process',array('PROCESS_ID'=>$row->PROCESS_ID));
      ...
      $option = '<a  href="#"  data-toggle="modal" data-target="#mystatut'.$row->ID.'" title="Supprimer"  class="btn btn-danger"> <span class="fa fa-trash"></span></a>


        <div class="modal fade" id="mystatut' . $row->ID . '" >
                          <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                <a class="" href="' . base_url('administration/Affect_Documents/delete/'.$row->ID) . '"><span class="mode mode_on">SUPPRIMER</span></a>
    ...
    {
     $this->Model->delete('pms_documents_stage',array('ID'=>$id));
